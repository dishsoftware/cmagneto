...
namespace Dish::ContactHolder::Contacts::FieldType {
    ...
    const QString& toString(const Enum iEnum) {
        ...
        Q_ASSERT_X(it != allValues().left.end(), "Dish::ContactHolder::Contacts::FieldType::toString", "Reconcile Enum and allValues() implementation!");
        return it->second;
    }
    ...
    Enum toEnum(const QString& iStr) {
        ...
        if (it == allValues().right.end())
            throw std::invalid_argument("Dish::ContactHolder::Contacts::FieldType::toEnum: invalid string representation \"" + iStr.toStdString() + "\"");
