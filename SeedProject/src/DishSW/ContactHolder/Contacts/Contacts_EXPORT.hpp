#pragma once
        ...
        #define COMPILE_TIME_MESSAGE(msg) /* Unsupported compiler */
...
#if defined(LIB_DISHSW_CONTACTHOLDER_CONTACTS_SHARED)
    #if defined(DISHSW_CONTACTHOLDER_CONTACTS_EXPORTS) || defined(DishSW_ContactHolder_Contacts_EXPORTS)
        #if defined(_WIN32)
            #if defined(__GNUC__)
                #define DISHSW_CONTACTHOLDER_CONTACTS_EXPORT __attribute__((visibility("default")))
                #pragma message ("MinGW Export")
            #elif defined(_MSC_VER)
                #define DISHSW_CONTACTHOLDER_CONTACTS_EXPORT __declspec(dllexport)
                COMPILE_TIME_MESSAGE("MSVC Export")
            #else
                #define DISHSW_CONTACTHOLDER_CONTACTS_EXPORT
            ...
            #elif defined(_MSC_VER)
                #define DISHSW_CONTACTHOLDER_CONTACTS_EXPORT __declspec(dllimport)
