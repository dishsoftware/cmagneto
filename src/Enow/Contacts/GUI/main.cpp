...
int main() {
    ...
    std::wcout << "Enow::Contacts::Contacts::FieldType::Enum::kEMailAddress index: " << static_cast<int>(fieldType) << std::endl;

    const auto& fieldTypeString = Enow::Contacts::Contacts::FieldType::toString(fieldType);
    std::wcout << "Enow::Contacts::Contacts::FieldType::toString(kEMailAddress): " << fieldTypeString.toStdWString() << std::endl;
