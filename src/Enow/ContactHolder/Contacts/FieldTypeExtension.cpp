...
namespace Enow::ContactHolder::Contacts::FieldType {
    ...
    const QString& toString(const Enum iEnum) {
        ...
        Q_ASSERT_X(it != allValues().left.end(), "Enow::ContactHolder::Contacts::FieldType::toString", "Reconcile Enum and allValues() implementation!");
        return it->second;
    }
    ...
    Enum toEnum(const QString& iStr) {
        ...
        if (it == allValues().right.end())
            throw std::invalid_argument("Enow::ContactHolder::Contacts::FieldType::toEnum: invalid string representation \"" + iStr.toStdString() + "\"");
